...
author: "Taylor Jancetic and Chloe Hampson"
date: "2024-09-30"
output:
  html_document:
    fontsize : 9 pt
...
```{r}
# load datasets
covar_df <- read_csv("/Users/chloehampson/Desktop/projects/abcd-plsc/derivatives/covariate.csv")
socio_df <- read_csv("/Users/chloehampson/Desktop/projects/abcd-plsc/derivatives/sociocult.csv")
rsfc_df <- read_csv("/Users/chloehampson/Desktop/projects/abcd-plsc/derivatives/rsfc.csv")
...
```{r}
# Extract all the variables used in the model
variables <- list(
  interview_age = Group1.X$interview_age,
  demo_sex_v2 = as.factor(Group1.X$demo_sex_v2),
  demo_prnt_age_v2 = Group1.X$demo_prnt_age_v2,
  demo_prnt_gender_id_v2 = as.factor(Group1.X$demo_prnt_gender_id_v2),
  demo_prnt_ed_v2_2yr_l = Group1.X$demo_prnt_ed_v2_2yr_l,
  demo_prtnr_ed_v2_2yr_l = Group1.X$demo_prtnr_ed_v2_2yr_l,
  demo_comb_income_v2 = Group1.X$demo_comb_income_v2,
  demo_prnt_income_v2_l = Group1.X$demo_prnt_income_v2_l,
  mri_info_manufacturer = as.factor(Group1.X$mri_info_manufacturer),
  rsfmri_meanmotion = Group1.X$rsfmri_meanmotion
)

# Function to check if a factor/numeric variable has at least 2 levels
check_levels <- function(var) {
  if (is.factor(var)) {
    # Get levels and counts for factor
    counts <- table(var)
    has_at_least_2_levels <- length(counts) >= 2
    return(list(has_at_least_2_levels = has_at_least_2_levels, counts = counts))
  } else if (is.numeric(var)) {
    # Get unique values and their counts for numeric
    unique_vals <- unique(var[!is.na(var)])
    counts <- table(var)
    has_at_least_2_unique <- length(unique_vals) >= 2
    return(list(has_at_least_2_unique = has_at_least_2_unique, counts = counts))
  } else {
    return(NA) # If not a factor or numeric, return NA
  }
}

# Apply the function to all variables
result <- sapply(variables, check_levels)
...
corrplot(XY.cor.pearson,
  method = "color", tl.cex = 1, tl.col = col4rsi,
