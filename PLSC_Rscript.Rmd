...
fig_dir <- "/Users/chloehampson/Desktop/projects/abcd-plsc/figures/"
...
```{r echo=TRUE}
print(colnames(covar_df)) # Print all column names to confirm the presence of site_id_l
print(head(covar_df$site_id_l)) # Print the first few values of the site_id_l column
...
grid.arrange(score2, score3, score4, nrow = 1)
...
plotSalience_fn <- function(set, dimension) {
 ...
 # Set the file path for saving the plot
 png(file.path(fig_dir, "Salience_XDim4.png"), width = 1800, height = 1200, res = 200)

 plotSalience_fn(signed.X, 4)

 # Close the PNG device to save the file
 dev.off()

  png(file.path(fig_dir, "Salience_YDim4.png"), width = 1800, height = 1200, res = 200)

 plotSalience_fn(signed.Y, 4)
