...
for (dim in dimensions_to_plot) {
  ...
  # For the *first* RSFC plot (Dim 1), make only the significant bars black
  # (significant = abs(bootstrap ratio) >= threshold) and color their labels black too
  if (dim == 1 && inherits(plot_BRI, "ggplot")) {
    sig_df <- subset(plot_BRI$data, abs(bootratio) >= thr)
    if (nrow(sig_df) > 0) {
      plot_BRI <- plot_BRI +
        geom_col(
          data = sig_df,
          mapping = aes(x = IDnum, y = bootratio),
          inherit.aes = FALSE,
          fill = "black",
          colour = "black"
        ) +
        geom_text(
          data = sig_df,
          mapping = aes(x = IDnum, y = 0, label = ID),
          inherit.aes = FALSE,
          colour = "black",
          size = 4,
          vjust = 1
