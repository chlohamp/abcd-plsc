...
```{r}
# load datasets
covar_df <- read_csv("/Users/chloehampson/Desktop/abcd-plsc/derivatives/none-reduced-no_auditory/covariate.csv")
socio_df <- read_csv("/Users/chloehampson/Desktop/abcd-plsc/derivatives/none-reduced-no_auditory/clean-socio.csv")
rsfc_df <- read_csv("/Users/chloehampson/Desktop/abcd-plsc/derivatives/none-reduced-no_auditory/clean-rsfc.csv")


fig_dir <- "/Users/chloehampson/Desktop/abcd-plsc/derivatives/none-reduced-no_auditory/figures/"
dir.create(fig_dir, showWarnings = FALSE, recursive = TRUE)
out_dir <- "/Users/chloehampson/Desktop/abcd-plsc/derivatives/none-reduced-no_auditory"
...
# Create the plot
png(corr, width = 1600, height = 400, res = 300) # Increase res to 300 dpi
...
# Colors are keyed by VARIABLE NAME (not row position), so they stay attached
# to the correct variable no matter what order the bars are plotted in.
socio_color_map <- setNames(rep("black", ncol(data2)), colnames(data2))
socio_color_map["COI"]   <- "#6FBFE1"
socio_color_map["NDI"]   <- "#9BF1FE"
socio_color_map["FO-C"]  <- "#90CEC1"
socio_color_map["EIE-C"] <- "#90CEC1"
socio_color_map["FR-C"]  <- "#BCE3C8"
socio_color_map["EIC-C"] <- "#BCE3C8"
socio_color_map["HA-C"]  <- "#D2F3D8"

# Desired x-axis order for the sociocultural (BR.J) plot:
# Youth-report block, then Caregiver-report block, then neighborhood/environment block
socio_order <- c(
...
# Reorder BR.J's rows and pull colors in that same order - since both are
# indexed by the same `socio_order` vector, they can never get out of sync.
BR.J <- BR.J[socio_order, , drop = FALSE]
socio_colors <- unname(socio_color_map[socio_order])

rsfc_colors <- rep("black", nrow(BR.I))
...
write.csv(
  variance_table,
  file = file.path(out_dir, "percent_variance_explained.csv"),
  row.names = FALSE
)
